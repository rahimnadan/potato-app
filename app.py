...
# html("""
#     <script>
#     const streamlitEvents = new EventSource('/_stcore_streamlit_events');
#     streamlitEvents.addEventListener('tab', function(event) {
#         const data = JSON.parse(event.data);
#         document.querySelector('.nav-tabs a.active').classList.remove('active');
#         document.querySelector(`.nav-tabs a[onclick="navigateTo('${data.tab}')"]`).classList.add('active');
#         document.getElementById(data.tab).scrollIntoView({behavior: 'smooth'});
#     });
#     </script>
# """)
...
#         image = Image.open(uploaded_file)
#         st.image(image, caption='Uploaded Image.', use_column_width=True)
#     if st.button('Predict', key='predict_button', help="Click to predict the disease"):
#         st.write("Classifying...")

#         if uploaded_file is not None:
#             result = infer_image(image)
#         elif image_url:
#             result = infer_image_from_url(image_url)
